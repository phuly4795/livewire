<div wire:key='{{ $item->id }}' class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            ...
            @if ($item->completed == 1)
                <input wire:click='toggle({{ $item->id }})' type="checkbox" class="mr-2" checked>
            @else
                <input wire:click='toggle({{ $item->id }})' type="checkbox" class="mr-2">
            @endif

            @if ($edittingToDoId === $item->id)
                <div>
                    <input wire:model='edittingToDoName' type="text" placeholder="Todo.."
            ...
            @else   
                <h3 class="text-lg text-semibold text-gray-800" 
                    @if ($item->completed == 1) style="text-decoration-line: line-through; color:#ccc" @endif>
                    {{ $item->name }}
                </h3>          
            @endif
        </div>
